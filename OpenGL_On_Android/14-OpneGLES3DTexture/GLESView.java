package com.example.sknaa.OGLES3DTexture;
...
private void initialize(GL10 gl)
{
	...
	ByteBuffer byteBufferColor = ByteBuffer.allocateDirect(pyramidTexCoords.length * 4);
	...
	GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER, pyramidTexCoords.length * 4, colorBuffer, GLES31.GL_STATIC_DRAW);
	...
	ByteBuffer byteBufferColorCube = ByteBuffer.allocateDirect(CubeTexcoords.length * 4);
	byteBufferColorCube.order(ByteOrder.nativeOrder());
	FloatBuffer colorBufferCube = byteBufferColorCube.asFloatBuffer();
	colorBufferCube.put(CubeTexcoords);
	colorBufferCube.position(0);
	GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER, CubeTexcoords.length * 4, colorBufferCube, GLES31.GL_STATIC_DRAW);
	GLES31.glVertexAttribPointer(GLESMacros.VDG_ATTRIBUTE_TEXTURE, 2, GLES31.GL_FLOAT, false, 0, 0);
	GLES31.glEnableVertexAttribArray(GLESMacros.VDG_ATTRIBUTE_TEXTURE);
	GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER, 0);
	GLES31.glBindVertexArray(0);
...
void uninitialize()
{
	...
	{
		GLES31.glDeleteVertexArrays(1, vbo_Pyramid_Texture, 0);
	...
	{
		GLES31.glDeleteVertexArrays(1, vbo_Cube_Texture, 0);
	...
	{
		GLES31.glDeleteTextures(1, texture_stone, 0);
		texture_stone[0] = 0;
	}

	if(texture_kundali[0] != 0)
	{
		GLES31.glDeleteTextures(1, texture_kundali, 0);
		texture_kundali[0] = 0;
	}
