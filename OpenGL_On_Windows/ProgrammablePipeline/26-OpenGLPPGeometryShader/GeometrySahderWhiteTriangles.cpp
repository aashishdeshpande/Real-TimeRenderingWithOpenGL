...
void initialize(void)
{
	...
	//choose a pixel format which best matches with pfd
	...
	//provide source code to the shader
	...
	glGetShaderiv(gFragmentShaderObject, GL_COMPILE, &iShaderCompiledStatus);
	if (iShaderCompiledStatus == GL_FALSE)
	{
	...
	glBindAttribLocation(gShaderProgramObject, ARD_ATTRIBUTE_VERTEX, "vPosition");
	...
	glVertexAttribPointer(ARD_ATTRIBUTE_VERTEX, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(ARD_ATTRIBUTE_VERTEX);
