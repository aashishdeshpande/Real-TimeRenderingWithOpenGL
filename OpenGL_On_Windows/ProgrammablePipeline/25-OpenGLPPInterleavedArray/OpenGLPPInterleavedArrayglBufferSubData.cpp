...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szClassName[] = TEXT("OpenGLPP Interleaved Array");
	bool bDone = false;
...
void initialize(void)
{
	...
	glBufferData(GL_ARRAY_BUFFER, 24 * 11 * sizeof(float), NULL, GL_STATIC_DRAW);
	
	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(cubeVertices), &cubeVertices);
	glVertexAttribPointer(ARD_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(ARD_ATTRIBUTE_POSITION);

	glBufferSubData(GL_ARRAY_BUFFER, sizeof(cubeVertices), sizeof(cubeColor), &cubeColor);
	glVertexAttribPointer(ARD_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 0, (void *)(sizeof(cubeVertices)));
	glEnableVertexAttribArray(ARD_ATTRIBUTE_COLOR);
	
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(cubeColor) + sizeof(cubeVertices), sizeof(cubeNormal), &cubeNormal);
	glVertexAttribPointer(ARD_ATTRIBUTE_NORMAL, 3, GL_FLOAT, GL_FALSE, 0, (void *)(sizeof(cubeVertices) + sizeof(cubeColor)));
	glEnableVertexAttribArray(ARD_ATTRIBUTE_NORMAL);

	glBufferSubData(GL_ARRAY_BUFFER, sizeof(cubeVertices) + sizeof(cubeColor) + sizeof(cubeNormal), sizeof(cubeTexCoords), &cubeTexCoords);
	glVertexAttribPointer(ARD_ATTRIBUTE_TEXTURE0, 2, GL_FLOAT, GL_FALSE, 0, (void *)(sizeof(cubeVertices) + sizeof(cubeColor) + sizeof(cubeNormal)));
